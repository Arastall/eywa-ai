...
const tripadvisor_js_1 = require("../services/tripadvisor.js");
// Store original env
...
describe('parseRankingString', () => {
    test('parses standard ranking format', () => {
        const result = (0, tripadvisor_js_1.parseRankingString)('#15 of 234 Hotels in Paris');
    ...
    test('parses ranking with different locations', () => {
        const result = (0, tripadvisor_js_1.parseRankingString)('#3 of 50 Hotels in Bangkok');
        expect(result.position).toBe(3);
        expect(result.total).toBe(50);
        expect(result.context).toBe('of 50 Hotels in Bangkok');
    });
    test('handles unrecognized format', () => {
        const result = (0, tripadvisor_js_1.parseRankingString)('Top Rated Hotel');
        expect(result.position).toBe(0);
        expect(result.total).toBe(0);
        expect(result.context).toBe('Top Rated Hotel');
    });
    test('handles empty string', () => {
        const result = (0, tripadvisor_js_1.parseRankingString)('');
        expect(result.position).toBe(0);
        expect(result.total).toBe(0);
