...
exports.calculateTrendFromScores = calculateTrendFromScores;
...
exports.generateTimelineData = generateTimelineData;
exports.calculateReviewRate = calculateReviewRate;
 ...
 */
function calculateTrendFromScores(scores, period) {
    const now = new Date();
 ...
 */
function detectAlerts(currentScore, previousScore, recentReviews, normalReviewRate // reviews per day
) {
    const alerts = [];
    const now = new Date();
    ...
    if (previousScore !== null) {
        ...
        if (scoreDelta <= exports.ALERT_THRESHOLDS.SCORE_DROP_HIGH) {
            alerts.push({
                type: 'score_drop',
                severity: 'high',
        ...
        else if (scoreDelta <= exports.ALERT_THRESHOLDS.SCORE_DROP_MEDIUM) {
            alerts.push({
                type: 'score_drop',
                severity: 'medium',
                message: `Eywa Score dropped by ${Math.abs(scoreDelta).toFixed(2)} points`,
        ...
        else if (scoreDelta >= exports.ALERT_THRESHOLDS.SCORE_RISE_HIGH) {
            alerts.push({
                type: 'score_rise',
                severity: 'high',
        ...
        else if (scoreDelta >= exports.ALERT_THRESHOLDS.SCORE_RISE_MEDIUM) {
            alerts.push({
                type: 'score_rise',
                severity: 'medium',
                message: `Eywa Score improved by ${scoreDelta.toFixed(2)} points`,
    ...
    if (normalReviewRate > 0 && last24hReviews.length >= normalReviewRate * exports.ALERT_THRESHOLDS.REVIEW_SPIKE_FACTOR) {
        alerts.push({
            type: 'review_spike',
            severity: 'medium',
    ...
    const negativeReviews = last24hReviews.filter(r => r.rating <= exports.ALERT_THRESHOLDS.NEGATIVE_REVIEW_RATING);
 ...
 */
function calculateMarketPosition(hotelScore, competitorScores) {
 ...
 */
function generateTimelineData(scores, period = '30d') {
    const periodDays = parseInt(period);
    const now = new Date();
    const startDate = new Date(now.getTime() - periodDays * 24 * 60 * 60 * 1000);
 ...
 */
function calculateReviewRate(reviews, days = 30) {
    const now = new Date();
