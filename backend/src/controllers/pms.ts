...
const PMS_ENDPOINTS: Record<string, any> = {
  mews: {
    name: 'Mews',
...
export const getPMSTypes = async (_req: AuthRequest, res: Response) => {
  res.json(
    Object.entries(PMS_ENDPOINTS).map(([slug, data]) => ({
      slug,
      name: data.name,
...
export const getConnection = async (req: AuthRequest, res: Response) => {
  try {
    const result = await query(
      `SELECT id, pms_type, environment, endpoint_url, last_sync_at, sync_status, sync_error, is_active, created_at
...
export const connectPMS = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await query(
      'UPDATE pms_connections SET is_active = false WHERE hotel_id = $1',
      [hotelId]
    );
    ...
    const result = await query(
      `INSERT INTO pms_connections (hotel_id, pms_type, environment, endpoint_url, client_token, access_token, sync_status)
       VALUES ($1, $2, $3, $4, $5, $6, 'pending')
       RETURNING *`,
      [hotelId, pms_type, environment, endpoint_url, client_token, access_token]
    );

    // Update hotel pms_type
    await query('UPDATE hotels SET pms_type = $1 WHERE id = $2', [pms_type, hotelId]);
...
export const testEndpoint = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const connResult = await query(
      'SELECT * FROM pms_connections WHERE hotel_id = $1 AND is_active = true LIMIT 1',
      [hotelId]
    );
    ...
    // Log the API call
    await query(
      `INSERT INTO api_logs (hotel_id, user_id, endpoint, method, request_body, response_code, response_body, latency_ms)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [hotelId, req.user?.id, endpoint, method, body, response.status, responseData, latency]
    );
...
export const getApiLogs = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await query(
      `SELECT id, endpoint, method, response_code, latency_ms, created_at
       FROM api_logs
       WHERE hotel_id = $1
       ORDER BY created_at DESC
       LIMIT $2`,
      [req.user?.hotel_id, Number(limit)]
    );
