# EYWA AI - Database Schema (MERISE)
...
└─────────────────┘               ▼                         ▼
                          ...
                          └─────────────────┘       │ total_revenue   │
                                                    ...
                                                    └─────────────────┘
                                                            ...
                                                            ▼
┌─────────────────┐       ┌─────────────────┐       ┌─────────────────┐
│  DAILY_STATS    │       │  MONTHLY_REPORT │       │   ROI_METRIC    │
├─────────────────┤       ├─────────────────┤       ├─────────────────┤
│ id (PK)         │       │ id (PK)         │       │ id (PK)         │
│ hotel_id (FK)   │       │ hotel_id (FK)   │       │ hotel_id (FK)   │
│ date            │       │ year_month      │       │ period_start    │
│ occupancy_rate  │       │ total_revenue   │       │ period_end      │
│ adr             │       │ direct_revenue  │       │ eywa_cost       │
│ revpar          │       │ ota_revenue     │       │ revenue_gained  │
│ direct_bookings │       │ ai_cost         │       │ time_saved_hrs  │
│ ota_bookings    │       │ eywa_fee        │       │ roi_percentage  │
│ ai_interactions │       │ net_profit      │       │ calculated_at   │
│ created_at      │       │ roi_vs_baseline │       └─────────────────┘
└─────────────────┘       │ created_at      │
                          └─────────────────┘

┌─────────────────┐       ┌─────────────────┐
│   API_LOG       │       │  NOTIFICATION   │
├─────────────────┤       ├─────────────────┤
│ id (PK)         │       │ id (PK)         │
│ hotel_id (FK)   │       │ hotel_id (FK)   │
│ endpoint        │       │ user_id (FK)    │
...
- UUIDs pour toutes les PKs (sécurité + distributed-friendly)
- TIMESTAMPTZ pour toutes les dates (timezone-aware)
- JSONB pour les données flexibles (vs_baseline, request/response)
- Soft delete via `is_active` flag plutôt que DELETE
